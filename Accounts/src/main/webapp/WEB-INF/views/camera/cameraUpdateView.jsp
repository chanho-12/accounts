...
<hr>
<h2 align="center">${ camera.camera_num } 번 영수증 수정 페이지</h2>
<br>
<!-- 영수증 수정 form -->
<!-- form에서 입력된 값들과 첨부파일을 같이 전송하려면, 반드시 enctype 속성을 추가해야 함
   enctype="multipart/form-data" 값을 지정해야 함 -->
<form id= "submitForm" action="cup.do" method="post" enctype="multipart/form-data">
<input type="hidden" name="camera_num" value="${ camera.camera_num }">
<input type="hidden" name="page" value="${ currentPage }">

<c:if test="${  !empty camera.original_filepath}">
   <input type="hidden" name="original_filepath" value="${ camera.original_filepath }">
   <input type="hidden" name="rename_filepath" value="${ camera.rename_filepath }">

</c:if>
<table align="center" width="500" border="1" cellspacing="0" cellpadding="5">
   <tr><th>상호명</th><td><input type="text" name="camera_storename" value="${ camera.camera_storename }"></td></tr>
   <tr><th>첨부파일</th>
      <td>
      <!-- 원래 첨부파일이 있는 경우 -->
      <c:if test="${ !empty camera.original_filepath }">
         ${ camera.original_filepath } &nbsp; 
         <input type="checkbox" name="delFlag" value="yes"> 파일삭제 <br>
      </c:if>
      <br>
      새로 첨부 : <input type="file" name="upfile">      
      </td></tr>
   <tr><th>합계금액</th><td><input type="text" name="camera_total" value="${ camera.camera_total }"></td></tr>
...
<tr><th width="200px">품명</th><th width="200px">수량</th><th width="200px">단가</th><th width="200px">금액</th></tr>
 <c:forEach items = "${ product }" var = "product">
    ...
    <input type="hidden" name="product_no" value="${ product.product_no }">
      <td align="center"><input type="text" name="product_name" value="${product.product_name}"></td>
      <td align="center"><input type="text" name="product_count" value="${product.product_count}"></td>
      <td align="center"><input type="text" name="unit_price" value="${product.unit_price}"></td>
      <td align="center"><input type="text" name="product_price" value="${product.product_price}"></td>
    </tr>
</c:forEach>
</table>
</td></tr>
      <tr><td colspan="2">
      <input type="submit" onclick="submitclick" value="수정하기"> &nbsp; 
